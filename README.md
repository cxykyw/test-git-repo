...
- `ai_review.py`：取自上次已审基点到当前 HEAD 的累计 diff（基点记录在 workspace 的 `.ai-review-base`，整段范围审完才推进，多次提交攒着一次构建也不会漏审；基点不可用时回退 `GIT_PREVIOUS_COMMIT` / `HEAD~1`），按文件切分片（默认 100K 字符/片，`AI_REVIEW_CHUNK_CHARS` 可调）并行送审（`AI_REVIEW_WORKERS`，默认 3），结果聚合去重；lock/构建产物/生成代码自动跳过，单文件超长截断并在结果中标注，超出分片上限的文件同样如实标记。默认 advisory 模式（只展示不阻断）；Jenkins 环境变量设 `DIFY_BLOCKING=1` 后，仅 blocker/critical 级发现或未审完的分片才阻断打包，major 以下只展示（阻断级别可用 `AI_REVIEW_BLOCK_SEVERITIES` 覆盖）
